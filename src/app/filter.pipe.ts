  ...
  transform(objs: Array<unknown>, key, values?, targetNestedArray?): unknown {
    ...
    if (typeof key == 'string') {
      if (Array.isArray(values)) {
        ...
        return objs.filter(o => {
          if (targetNestedArray) return o[targetNestedArray].filter(e => e[key as string] == values).length > 0
      ...
      for (let k of key) {
        if (Array.isArray(values)) {
          result.push(...objs.filter(o => {
            if (targetNestedArray) return o[targetNestedArray].filter(e => values.includes(e[k])).length > 0
            return values.includes(o[k])
          }))
        }
        else {
          result.push(...objs.filter(o => {
            if (targetNestedArray) return o[targetNestedArray].filter(e => e[k] == values).length > 0
            return o[k] == values
          }))
        }
      }
